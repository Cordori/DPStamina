...
public class Region {
    ...
    public static HashMap<String, Region> regionHashMap = new HashMap<>();
    ...
    public static boolean isPlayerInRegion(Player player) {
        ...
        if(!regionHashMap.containsKey(world)) return false;

        double playerX = playerLoc.getX();
        double playerY = playerLoc.getY();
        double playerZ = playerLoc.getZ();

        for (String worldName : regionHashMap.keySet()) {

            if(!worldName.equals(world)) continue;

            String[] pos1 = regionHashMap.get(worldName).pos1;
            String[] pos2 = regionHashMap.get(worldName).pos2;
